...
Averageestimateplot2 =
  ...
  ggplot(aes(x = mu, y = mu_hat, colour = type)) +
  geom_line() +
  geom_point() +
...
Overall sample mean of μ̂ across all tests is approximately equal to the
true value μ, because μ̂ is an unbiased estimator. Between the two
summaries we consider, this unconditional sample mean should therefore
be the most accurate prediction of μ. In contrast, when we take the
sample average of μ̂ only over tests for which the null hypothesis is
rejected, this conditional mean tends to be larger than the true μ,
since small estimates are unlikely to lead to rejection and are
effectively excluded. As the true value μ increases, the power of the
test becomes larger, so it is harder to accept the null hypothesis and
fewer small μ̂ values are filtered out. As a result, the sample average
of μ̂ given rejection moves closer to the true μ, which matches the
behavior shown in the plot.
